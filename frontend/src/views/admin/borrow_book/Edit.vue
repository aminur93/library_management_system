...
export default {
  ...
  computed: {
    ...mapState({
      editBorrowBook: state => state.borrow_book.borrowBook,
  ...
  mounted() {
    this.getAllBooks();
    this.getAllMembers();
    this.getSingleBorrowBook(this.$route.params.id);
  },
  ...
  methods: {
    ...mapActions({
      ...
      getSingleBorrowBook: "borrow_book/GetSingleBorrowBook"
    }),

    updateBorrowBook: async function(){
      try {
        let id = this.$route.params.id;
        let formData = new FormData();

        formData.append('book_id', this.editBorrowBook.book_id);
        formData.append('member_id', this.editBorrowBook.member_id);
        formData.append('borrow_date', this.editBorrowBook.borrow_date);
        formData.append('return_date', this.editBorrowBook.return_date);
        formData.append('status', this.editBorrowBook.status);

        await this.$store.dispatch('borrow_book/UpdateBorrowBook', {id:id, data:formData}).then(() => {
      ...
      <v-col cols="12">
        <v-row>
          <v-col cols="12" md="12" sm="12" lg="12">
            <v-card>
              <v-card-title><h3>Edit Borrow Book</h3></v-card-title>

              <v-divider></v-divider>

              <v-card-text>
                <v-form v-on:submit.prevent="updateBorrowBook">
                  <v-col cols="12">
                    <v-row wrap>
                      ...
                      <v-col cols="12" class="d-flex">
                        <v-row wrap>
                          <v-col cols="12" md="8" sm="12" lg="6">
                            <v-select
                                variant="outlined"
                                v-model="editBorrowBook.book_id"
                                :items="books"
                                item-title="title"
                                item-value="id"
                                label="select Book"
                          ...
                          <v-col cols="12" md="8" sm="12" lg="6">
                            <v-select
                                variant="outlined"
                                v-model="editBorrowBook.member_id"
                                :items="members"
                      ...
                      <v-col cols="12" class="d-flex">
                        <v-row wrap>
                          <v-col cols="12" md="6" sm="6" lg="6">
                            <v-text-field
                                type="date"
                                v-model="editBorrowBook.borrow_date"
                                label="Borrow Date"
                                persistent-hint
                                variant="outlined"
                                required
                          ...
                          <v-col cols="12" md="6" sm="6" lg="6">
                            <v-text-field
                                type="date"
                                v-model="editBorrowBook.return_date"
                                label="Return Date"
                                persistent-hint
                                variant="outlined"
                                required
